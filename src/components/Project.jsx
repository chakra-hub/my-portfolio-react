import React from 'react'

const Project = ({img,name,desc,language,link}) => {
    console.log(img,name,desc,language,link)
  return (
    <div className="project_detail">
                <img className="project_left" src={img}>

                </img>
                <div className="project_right">
                    <div className="project_heading">
                    {name}
                    </div>
                    <div className="project_breif">
                        {desc}
                    </div>
                    <div className="project_language">
                        <div className='language_name'>{language.one}</div>
                        <div className='language_name'>{language.two}</div>
                        <div className='language_name'>{language.three}</div>
                    </div>
                    <div className="project_links">
                        <a href={link.code} target='_blank'><div className="code_link links">Code <img className='live_logo' src="data:image/png;base64," alt="" /></div></a> 
                        <a href={link.live} target='_blank'><div className="live_link links">Live Demo  <img className='live_logo' src="https://static.thenounproject.com/png/1598671-200.png" alt="" /></div></a>
                    </div>
                </div>
            </div>
  )
}

export default Project
